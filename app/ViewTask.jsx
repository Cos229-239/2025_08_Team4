...
// Other components like BackArrowIcon, EditIcon, etc. remain the same
...
const Field = ({ label, value, onChangeText, placeholder, multiline = false }) => (
    <View style={styles.fieldContainer}>
        ...
        <TextInput style={[styles.input, multiline && styles.inputMultiline]} value={value} onChangeText={onChangeText} placeholder={placeholder} placeholderTextColor="#BDBDBD" multiline={multiline}/>
    </View>
);
// -------------------------------------------------------------------
      ...
      useCallback(() => {
        async function loadTask() {
          ...
          try {
            ...
            // CORRECTED: Read from the correctly spelled field name
        ...
        <ScrollView style={styles.screen}>
            <View style={styles.headerBubble}>
                ...
                <TouchableOpacity onPress={mode === 'edit' ? handleSave : () => setMode('edit')} style={styles.headerIconContainer} disabled={saving}>
                    {mode === 'edit' ? (<Text style={[styles.headerButtonText, { color: colors.primaryBlue, fontWeight: 'bold' }]}>{saving ? 'Saving...' : 'Save'}</Text>) : (<EditIcon color={colors.primaryBlue} size={24}/>)}
                </TouchableOpacity>
            </View>
                ...
                {mode === 'view' && (
                    <View style={styles.card}>
                        ...
                        <View style={styles.detailRow}>
                            ...
                            {/* CORRECTED: Use the new function and read from the correctly spelled field name */}
                            <View style={styles.detailItem}><Text style={styles.viewLabel}>Duration</Text><Text style={styles.value}>{formatDuration(currentData?.estimateMinutes)}</Text></View>
                        </View>
                        <View style={styles.divider} />
                    ...
                    <View style={styles.card}>
                        ...
                        <View style={styles.fieldContainer}>
                            ...
                            <Pressable onPress={() => setIsDurationPickerVisible(true)}>
                                <View style={styles.input}><Text style={{fontSize: 16, color: colors.textPrimary}}>{`${form.durationHours || 0} hr ${form.durationMinutes || 0} min`}</Text></View>
                            </Pressable>
                        </View>
                        <View style={styles.fieldContainer}>
                ...
                <Pressable style={styles.modalBackdrop} onPress={() => setIsDurationPickerVisible(false)}>
                    <Pressable style={styles.modalContent}>
                        <View style={styles.pickerContainer}>
                            <Picker style={styles.picker} selectedValue={form.durationHours} onValueChange={v => setField('durationHours', v)}>{Array.from({ length: 25 }, (_, i) => i).map(h => <Picker.Item key={h} label={`${h}`} value={h} />)}</Picker>
                            <Text style={styles.pickerLabel}>Hours</Text>
                            <Picker style={styles.picker} selectedValue={form.durationMinutes} onValueChange={v => setField('durationMinutes', v)}>{Array.from({ length: 12 }, (_, i) => i * 5).map(m => <Picker.Item key={m} label={`${m}`} value={m} />)}</Picker>
...
const styles = StyleSheet.create({
  ...
  card: { backgroundColor: colors.cardBackground, borderRadius: 24, padding: 20, shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.05, shadowRadius: 15, elevation: 5, marginBottom: 20, },
  cardSection: { paddingVertical: 10 },
  taskTitleHeader: { fontFamily: 'Pacifico-Regular', fontSize: 28, color: colors.textPrimary, marginBottom: 8, },
  ...
  headerButtonText: { fontFamily: 'OpenSans-SemiBold', fontSize: 16, color: colors.textPrimary, },
  ...
  input: { backgroundColor: "#fff", borderWidth: 1, borderColor: "#e5e7eb", borderRadius: 12, padding: 14, fontSize: 16, fontFamily: 'OpenSans-Regular', color: colors.textPrimary, justifyContent: 'center', minHeight: 54, },
  ...
  picker: { flex: 1, },
  pickerLabel: { fontSize: 16, fontFamily: 'OpenSans-SemiBold', },
  modalButton: { marginTop: 20, backgroundColor: colors.primaryBlue, borderRadius: 12, padding: 14, alignItems: 'center', },
  modalButtonText: { color: 'white', fontSize: 16, fontFamily: 'OpenSans-SemiBold', },
});