...
import { View, Text, ScrollView, StyleSheet, TouchableOpacity, TextInput, Alert, ActivityIndicator, FlatList } from 'react-native';
...
import { listMyProjects } from '../lib/projectRepo';
...
/** Read current project id from either relation or mirror string */
const currentProjectIdFromGoal = (g) => {
  if (!g) return null;
  if (Array.isArray(g.project)) {
    const v = g.project[0];
    if (!v) return null;
    return typeof v === 'string' ? v : v.$id;
  }
  if (g.project && typeof g.project === 'object' && g.project.$id) return g.project.$id;
  if (typeof g.project === 'string') return g.project;
  if (g.projectId) return g.projectId;
  return null;
};

/** Build relation value respecting single vs multi schema */
const buildProjectRelationValue = (selectedId, currentServerValue) => {
  const isMulti = Array.isArray(currentServerValue);
  if (isMulti) return selectedId ? [selectedId] : [];
  ...
  <TouchableOpacity onPress={() => navigation.push('ViewTask', { taskId: task.$id, initialTask: task })}>
    <View style={styles.taskCard}>
      ...
      <View style={styles.taskContent}>
        ...
        {task.dueDate && <Text style={styles.taskDueDate}>Due: {task.dueDate.split('T')[0]}</Text>}
      </View>
...
export default function ViewGoalScreen() {
  ...
  const [form, setForm] = useState(initialGoal || null);
  ...
  const [projectValue, setProjectValue] = useState(() => currentProjectIdFromGoal(initialGoal) || 'none');
  const [projectItems, setProjectItems] = useState([{ label: 'Not Assigned', value: 'none' }]);

  // Scroll management for dropdown visibility
  const scrollRef = useRef(null);
  const [projectY, setProjectY] = useState(0);

  // keep only one picker open at a time
  useEffect(() => { if (reviewOpen) setProjectOpen(false); }, [reviewOpen]);
  useEffect(() => { if (projectOpen) setReviewOpen(false); }, [projectOpen]);

  // auto-scroll to project picker when opened so it's fully visible
  useEffect(() => {
    if (projectOpen && scrollRef.current) {
      requestAnimationFrame(() => {
        scrollRef.current.scrollTo({ y: Math.max(0, projectY - 80), animated: true });
      });
    }
  }, [projectOpen, projectY]);
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const selectedId = projectValue === 'none' ? null : String(projectValue);
      const payload = {
        ...(form || {}),
        reviewCadence: reviewValue,
        projectId: selectedId,                                   // mirror id if you use it elsewhere
        project: buildProjectRelationValue(selectedId, goal?.project), // proper relation value
      };
      if (payload.project && typeof payload.project === 'object' && payload.project.$id) {
        payload.project = payload.project.$id;
      }
      const updatedGoal = await upsertGoal(payload);
      setGoal(updatedGoal);
      setForm(updatedGoal);
      setMode('view');
    } catch (err) {
  ...
  const handleCancel = () => {
    ...
    setProjectValue(currentProjectIdFromGoal(goal) || 'none');
    setMode('view');
  };

  // Load goal by id (if needed)
  useEffect(() => {
    async function loadGoal() {
      ...
      try {
        ...
        setProjectValue(currentProjectIdFromGoal(fetchedGoal) || 'none');
      } catch (e) {
    ...
    (async () => {
      try {
        ...
        setProjectItems([
          ...
          ...(rows || []).map(p => ({ label: p.name || '(Untitled Project)', value: String(p.$id) })),
        ]);

        const current = currentProjectIdFromGoal(form || goal) || 'none';
  ...
  }, [form?.projectId, form?.project, goal?.projectId, goal?.project]);

  // Project name display (view mode)
  const projectName = useMemo(() => {
    if (goal?.project?.name) return goal.project.name;
    if (Array.isArray(goal?.project) && goal.project[0]?.name) return goal.project[0].name;

    const id = currentProjectIdFromGoal(goal);
    if (id && projects?.length) {
      const p = projects.find(x => String(x.$id) === String(id));
      if (p) return p.name || '(Untitled Project)';
    }
    if (typeof goal?.project === 'string' && goal.project.trim()) return goal.project.trim();
    return 'Not specified';
  }, [goal?.project, goal?.projectId, projects]);

  // Header buttons in edit mode
  useLayoutEffect(() => {
  ...
  }, [navigation, mode, form, saving, reviewValue, projectValue]);
      ...
      <View style={[styles.container, { flex: 1 }]}>
        {selectedIndex === 0 ? (
          <ScrollView
            ...
            contentContainerStyle={{ paddingBottom: 300 }} // space so dropdown can fully show
          >
            {mode === 'view' ? (
              <View style={styles.card}>
                      ...
                      <View style={styles.mountainRow}>
                        {[1, 2, 3, 4, 5].map((n) => (
                          <SnowyMountain
                            ...
                            width={32}
                            height={32}
                            mountainColor={n <= goal.priority ? getPriorityColor(n) : '#E0E0E0'}
                            snowColor={'#FFFFFF'}
                            strokeWidth={0}
                          />
                        ))}
                ...
                <View style={styles.cardSection}>
                  <Text style={styles.viewLabel}>Why is this important?</Text>
                  <Text style={styles.value}>{goal?.description || 'Not specified'}</Text>
                </View>
                <View style={styles.cardSection}>
                  <Text style={styles.viewLabel}>What will I gain?</Text>
                  <Text style={styles.value}>{goal?.why || 'Not specified'}</Text>
                </View>
                <View style={styles.cardSection}>
                  <Text style={styles.viewLabel}>Success looks like...</Text>
                  <Text style={styles.value}>{goal?.successCriteria || 'Not specified'}</Text>
                </View>
                <View style={styles.divider} />

                <View style={styles.cardSection}>
                  <Text style={styles.viewLabel}>Notes</Text>
                  <Text style={styles.value}>{goal?.notes || 'Not specified'}</Text>
                </View>

                {/* VIEW: Project from DB */}
                <View style={styles.cardSection}>
                  <Text style={styles.viewLabel}>Project</Text>
                ...
                <View style={styles.detailRow}>
                  <View style={styles.detailItem}>
                    <Text style={styles.viewLabel}>Target Date</Text>
                    <Text style={styles.value}>{goal?.targetDate?.split('T')[0]}</Text>
                  </View>
                  <View style={styles.detailItem}>
                    <Text style={styles.viewLabel}>Review Cadence</Text>
                    <Text style={styles.value}>{goal?.reviewCadence || 'Not specified'}</Text>
                  </View>
                </View>
              </View>
            ) : (
              <View style={styles.detailsBubble}>
                ...
                <View style={styles.fieldContainer}>
                  ...
                  <View style={styles.mountainRow}>
                    {[1, 2, 3, 4, 5].map((n) => (
                      <TouchableOpacity key={n} onPress={() => setField('priority', n)}>
                        <SnowyMountain
                          width={40}
                          height={40}
                          mountainColor={n <= (form?.priority ?? 0) ? getPriorityColor(n) : '#E0E0E0'}
                          snowColor={'#FFFFFF'}
                          strokeWidth={0}
                        />
                      </TouchableOpacity>
                ...
                <View style={styles.fieldContainer}>
                  ...
                  <TouchableOpacity style={styles.input} onPress={() => setDatePickerOpen(true)}>
                    <Text style={{ fontSize: 16, color: colors.textPrimary }}>{form?.targetDate?.split('T')[0]}</Text>
                  </TouchableOpacity>
                </View>
                <DateTimePickerModal
                  isVisible={datePickerOpen}
                  mode="date"
                  date={parseISODate(form?.targetDate)}
                  onConfirm={(d) => { setDatePickerOpen(false); setField("targetDate", toISODate(d)); }}
                  onCancel={() => setDatePickerOpen(false)}
                />

                {/* REVIEW CADENCE */}
                <View style={styles.pickerWrapperTop}>
                ...
                {/* PROJECT DROPDOWN (auto-scrolls when opened) */}
                <View
                  style={styles.pickerWrapperBottom}
                  onLayout={e => setProjectY(e.nativeEvent.layout.y)}
                >
...
const styles = StyleSheet.create({
  ...
  dropdownContainer: {
    borderColor: "#e5e7eb",
    borderRadius: 12,
    maxHeight: 260,        // internal scroll so it never becomes a full-screen modal
    elevation: 30,
    zIndex: 6000,
  },
  ...
  taskPriorityIndicator: { width: 6, height: '80%', borderRadius: 3, marginRight: 12 },
  taskContent: { flex: 1 },
