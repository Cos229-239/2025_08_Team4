...
export async function listTasksUpToDuration(minutes, { onlyActive = true } = {}) {
  const ownerId = (await getOrCreateProfile()).ownerId;
  const m = parseInt(minutes, 10);
  if (Number.isNaN(m)) throw new Error("Please provide a valid number of minutes.");

  const q = [
    Query.equal("ownerId", ownerId),
    // show tasks <= selected minutes
    Query.lessThanEqual("estimateMinutes", m),
    // ignore tasks without an estimate (optional)
    // If your Appwrite version supports it, prefer:
    // Query.isNotNull("estimateMinutes"),
    // otherwise require non-negative numbers:
    Query.greaterThanEqual("estimateMinutes", 0),
    // sort shortest first
    Query.orderAsc("estimateMinutes"),
  ];

  if (onlyActive) {
    q.push(Query.equal("status", ["todo", "in_progress", "paused"]));
  }
